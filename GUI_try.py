...
class PhotoViewer:
   def __init__(self, master):
       ...
       #master.bind("<Down>", self.eliminate_down)
   ...
   def choose_folder(self):
       ...
       if self.folder:
           ...
           self.labels_bbox_folder = os.path.join(self.data_folder, "labels_bbox")
   ...
   def up_arrow(self, master):
       self.auto_segment()
       #self.show_next()
       #self.show_mask_binary
       #self.show_image()
       #self.auto_segment()



   def show_mask_binary(self):
       imagen_pil = Image.fromarray(self.mask_binary.astype('uint8'))
       imagen_segmentada_tk = ImageTk.PhotoImage(imagen_pil)
       self.segmented_image = imagen_pil
       self.image_label.config(image=imagen_segmentada_tk)
       self.image_label.image = imagen_segmentada_tk
       self.image_label.config(image=self.self.mask_binary)
   ...
   def auto_segment(self):
       ...
       if files:
           ...
           superposed_mask = cv2.bitwise_and(imagen,imagen, mask= inverted_mask)

           #revisar
           imagen_pil = Image.fromarray(superposed_mask.astype('uint8'))
           imagen_segmentada_tk = ImageTk.PhotoImage(imagen_pil)
           ...
           for contorno in contornos:
                # Convertir el contorno en un polígono con menos aproximación
       ...
       print(yolo_annotation_bbox)

       #self.yolo_annotation_bbox_general.append(yolo_annotation_bbox)
   ...
   def save_annotations(self):       
        ...
        print(self.labels_bbox_folder)
        print(self.labels_folder)
